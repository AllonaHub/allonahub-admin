(function () {
  ...
  const state = {
    ...
    cache: {
      users: [],
      applications: [],
      maritime: { applications: [], listings: [], freightRequests: [], freightMatches: [] },
      partners: [],
      orders: [],
      tickets: [],
      proposals: [],
  ...
  const views = {
    ...
    maritime: { label: "Denizcilik Onayları", marker: "Onay" },
  ...
  function badge(value, tone) {
    ...
    const color = tone
      ...
      || (["warning", "review", "in_progress", "pending_review", "awaiting_payment", "pending_super_admin"].includes(normalized) ? "orange" : "")
  ...
  function renderMaritime(payload) {
    const applications = payload?.applications || [];
    const listings = payload?.listings || [];
    const freightRequests = payload?.freightRequests || [];
    const freightMatches = payload?.freightMatches || [];
    const approvedFreightPartners = applications.filter((item) => (
      item.status === "approved"
      && item.user_id
      && item.profile_role === "partner"
      && ["broker", "shipowner", "agency"].includes(item.partner_role)
    ));
    const partnerByUser = new Map(approvedFreightPartners.map((item) => [item.user_id, item]));
    const matchesByRequest = freightMatches.reduce((map, item) => {
      const current = map.get(item.freight_request_id) || [];
      current.push(item);
      map.set(item.freight_request_id, current);
      return map;
    }, new Map());
    const freightRows = freightRequests.map((item) => {
      const matches = matchesByRequest.get(item.id) || [];
      const matchLabel = matches.length
        ? matches.map((match) => {
          const partner = partnerByUser.get(match.partner_user_id);
          return `${partner?.company_name || match.partner_user_id}: ${normalizeStatus(match.status)}`;
        }).join(" / ")
        : "-";
      const canAssign = ["submitted", "in_review", "matching", "quoted"].includes(item.status) && approvedFreightPartners.length > 0;
      return `
        <tr>
          <td>${titleCell(item.reference_no, dateTime(item.created_at))}</td>
          <td>${titleCell(`${item.load_port || "-"} → ${item.discharge_port || "-"}`, `${item.quantity || "-"} ${item.quantity_unit || ""}`)}</td>
          <td>${escape(item.cargo_type || "-")}</td>
          <td>${badge(item.status)}</td>
          <td>${escape(matchLabel)}</td>
          <td>${canAssign ? `<button class="admin-btn admin-btn--gold" type="button" data-maritime-freight-assign="${escape(item.id)}">Partner Ata</button>` : "-"}</td>
        </tr>
      `;
    });
    const applicationRows = applications.map((item) => {
      ...
      return `
        <tr>
          <td>${titleCell(item.company_name, item.reference_no || item.contact_name)}</td>
          <td>${escape(item.partner_role || "-")}</td>
          <td>${badge(item.status)}</td>
          <td>${titleCell(item.city || "-", item.country || "")}</td>
          <td>${dateTime(item.created_at)}</td>
          <td>
            <span class="admin-actions">
              ${item.status === "pending" ? `<button class="admin-btn" type="button" data-maritime-application-review="start_review" data-id="${escape(item.id)}">İncele</button>` : ""}
              ${canReview ? `<button class="admin-btn admin-btn--gold" type="button" data-maritime-application-review="approve" data-id="${escape(item.id)}">Onayla</button>` : ""}
              ${canReview ? `<button class="admin-btn admin-btn--danger" type="button" data-maritime-application-review="reject" data-id="${escape(item.id)}">Reddet</button>` : ""}
            </span>
          </td>
        </tr>
      `;
    ...
    const listingRows = listings.map((item) => {
      ...
      return `
        <tr>
          ...
          <td>${badge(item.listing_type)}</td>
          <td>${badge(item.status)}</td>
          <td>${titleCell(item.location_label || "-", item.detail_label || "")}</td>
          <td>${dateTime(item.expires_at)}</td>
          <td>
            <span class="admin-actions">
              ${canReview ? `<button class="admin-btn admin-btn--gold" type="button" data-maritime-listing-review="approve" data-id="${escape(item.id)}">Yayınla</button>` : ""}
              ${canReview ? `<button class="admin-btn admin-btn--danger" type="button" data-maritime-listing-review="reject" data-id="${escape(item.id)}">Reddet</button>` : ""}
            </span>
          </td>
        </tr>
      `;
    ...
    $("#adminContent").innerHTML = [
      section(
        "Navlun Eşleştirme",
        "Onaylı broker, gemi sahibi ve acenteleri belirli navlun taleplerine ata",
        warningPanel() + table(["Talep", "Rota", "Yük", "Durum", "Eşleşmeler", "İşlem"], freightRows, "Navlun talebi bulunamadı.")
      ),
      section(
        "Denizcilik Partner Başvuruları",
        "MFA korumalı denizcilik partner yetkisi inceleme kuyruğu",
        table(["Şirket", "Rol", "Durum", "Konum", "Tarih", "İşlem"], applicationRows, "Denizcilik partner başvurusu bulunamadı.")
      ),
      section(
        "Denizcilik İlan Onayları",
        "Onaylanan kayıtlar aktif ve süresi dolmamışsa modül vitrinine açılır",
        table(["İlan", "Tür", "Durum", "Detay", "Bitiş", "İşlem"], listingRows, "Denizcilik ilanı bulunamadı.")
      )
    ].join("");
  }
  ...
  async function loadMaritime() {
    const data = await api(`/v1/ops-console/maritime?${queryParams()}`);
    state.cache.maritime = {
      applications: data.applications || [],
      listings: data.listings || [],
      freightRequests: data.freightRequests || [],
      freightMatches: data.freightMatches || []
    };
    state.warnings = data.warnings || [];
    renderMaritime(state.cache.maritime);
  }
  ...
  async function loadView(view) {
    ...
    try {
      ...
      if (state.view === "maritime") await loadMaritime();
  ...
  async function reviewMaritimeApplication(applicationId, decision) {
    const labels = {
      start_review: "İncelemeye Al",
      approve: "Partneri Onayla",
      reject: "Partneri Reddet"
    };
    const data = await openModal({
      title: labels[decision] || "Denizcilik Partner Kararı",
      message: "Karar audit kaydına işlenir; kullanıcı rolü bu işlemle otomatik yükseltilmez.",
      confirmText: labels[decision] || "Kaydet",
      danger: decision === "reject",
      fields: [
        { id: "review_note", label: "İnceleme notu", type: "textarea", required: decision === "reject", max: 800 }
      ]
    });
    if (!data) return;
    await api(`/v1/ops-console/maritime-partner-applications/${encodeURIComponent(applicationId)}/review`, {
      method: "PATCH",
      body: { decision, review_note: data.review_note || "" }
    });
    showToast("Denizcilik partner başvurusu güncellendi.");
    await loadMaritime();
  }

  async function reviewMaritimeListing(listingId, decision) {
    const data = await openModal({
      title: decision === "approve" ? "Denizcilik İlanını Yayınla" : "Denizcilik İlanını Reddet",
      message: decision === "approve"
        ? "İlan aktif duruma alınarak halka açık denizcilik vitrinine gönderilir."
        : "İlan yayınlanmaz ve inceleme notu partnere gösterilir.",
      confirmText: decision === "approve" ? "Yayınla" : "Reddet",
      danger: decision === "reject",
      fields: [
        { id: "review_note", label: "İnceleme notu", type: "textarea", required: decision === "reject", max: 800 }
      ]
    ...
    showToast(decision === "approve" ? "Denizcilik ilanı yayınlandı." : "Denizcilik ilanı reddedildi.");
    await loadMaritime();
  }

  async function assignMaritimeFreightPartner(requestId) {
    const partners = (state.cache.maritime.applications || []).filter((item) => (
      item.status === "approved"
      && item.user_id
      && item.profile_role === "partner"
      && ["broker", "shipowner", "agency"].includes(item.partner_role)
    ));
    if (!partners.length) throw new Error("Atanabilir onaylı denizcilik navlun partneri bulunmuyor.");
    const data = await openModal({
      title: "Navlun Partneri Ata",
      message: "Seçilen partner yalnız bu navlun talebini ve kendi teklifini görebilir.",
      confirmText: "Eşleştir",
      fields: [
        {
          id: "partner_user_id",
          label: "Onaylı partner",
          type: "select",
          required: true,
          options: partners.map((item) => ({
            value: item.user_id,
            label: `${item.company_name} · ${normalizeStatus(item.partner_role)}`
          }))
        },
        {
          id: "expires_at",
          label: "Eşleşme bitişi",
          inputType: "datetime-local",
          required: true,
          value: futureDateTimeInput(7)
        }
      ]
    });
    if (!data) return;
    const expiresAt = dateTimeInputToIso(data.expires_at);
    if (!expiresAt) throw new Error("Geçerli bir eşleşme bitişi seçin.");
    await api(`/v1/ops-console/maritime-freight/${encodeURIComponent(requestId)}/matches`, {
      method: "POST",
      body: {
    ...
    showToast("Navlun partner eşleşmesi kaydedildi.");
    await loadMaritime();
  }
  ...
  function bindEvents() {
    document.addEventListener("click", async (event) => {
      ...
      const maritimeApplicationReview = event.target.closest("[data-maritime-application-review]");
      if (maritimeApplicationReview) {
        await reviewMaritimeApplication(
          maritimeApplicationReview.dataset.id,
          maritimeApplicationReview.dataset.maritimeApplicationReview
        ).catch((error) => showToast(error.message, "error"));
        return;
      }
      const maritimeListingReview = event.target.closest("[data-maritime-listing-review]");
      if (maritimeListingReview) {
        await reviewMaritimeListing(
          maritimeListingReview.dataset.id,
          maritimeListingReview.dataset.maritimeListingReview
        ).catch((error) => showToast(error.message, "error"));
        return;
      }
      const maritimeFreightAssign = event.target.closest("[data-maritime-freight-assign]");
      if (maritimeFreightAssign) {
        await assignMaritimeFreightPartner(maritimeFreightAssign.dataset.maritimeFreightAssign)
          .catch((error) => showToast(error.message, "error"));
        return;
      }
