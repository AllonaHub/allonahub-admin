(function () {
  ...
  const marsohWindows1252Bytes = new Map([
  ...
  function repairMarsohEncoding(value) {
    let text = String(value || "");
    for (let pass = 0; pass < 2 && /\uFFFD|Ã|Ä|Å|Â|Ð|Ñ|â|É|™|Ÿ/.test(text); pass += 1) {
      const bytes = [];
      let convertible = true;
      for (const character of text) {
        const point = character.codePointAt(0);
        if (point <= 0xff) bytes.push(point);
        else if (marsohWindows1252Bytes.has(point)) bytes.push(marsohWindows1252Bytes.get(point));
        else {
          convertible = false;
          break;
        }
  ...
  function marsohLocalizedFields(prefix, values, options) {
    ...
    const sourceValue = escape(repairMarsohEncoding(values && values.tr || ""));
    ...
    const previews = Object.entries(marsohLanguageLabels)
      ...
      .map(([language, label]) => `<div class="sa-marsoh-translation-preview"><span>${escape(label)}</span><p lang="${escape(language)}" dir="${language === "ar" ? "rtl" : "auto"}">${escape(repairMarsohEncoding(values && values[language] || "Henüz oluşturulmadı"))}</p></div>`)
      .join("");
  ...
  function marsohChannelLabel(channel) {
    return repairMarsohEncoding(channel?.name_i18n?.tr || channel?.name_i18n?.az || channel?.name_i18n?.en || channel?.slug || "Oda");
  }
