...
export const config = {
  ...
  marsoh: {
    ...
    translationProvider: readEnv("MARSOH_TRANSLATION_PROVIDER", { required: false, defaultValue: "local" }).trim().toLowerCase(),
    translationLocalUrl: readEnv("MARSOH_LOCAL_TRANSLATION_URL", { required: false, defaultValue: "http://marsoh-translator:8080" }).replace(/\/$/, ""),
    translationLocalSecret: readOptionalSecret("MARSOH_LOCAL_TRANSLATION_SECRET"),
    translationLocalTimeoutMs: Math.max(5000, Math.min(readNumber("MARSOH_LOCAL_TRANSLATION_TIMEOUT_MS", 60000), 120000)),
