...
comment on function public.is_ops_admin() is
  'Strict Admin Panel role helper. Super Admin is intentionally excluded so the Admin Panel remains a limited operations surface.';
...
create policy "support_tickets_own_or_ops_admin_select"
  ...
  to authenticated
  using (user_id = auth.uid() or public.is_ops_admin() or public.is_super_admin());
...
create policy "support_tickets_ops_admin_update"
  ...
  to authenticated
  using (public.is_ops_admin() or public.is_super_admin())
  with check (public.is_ops_admin() or public.is_super_admin());
